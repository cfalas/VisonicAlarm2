...
class Device(object):
    ...
    def __init__(
        self, id, name, zone, device_type, subtype, preenroll, warnings, partitions
    ):
...
class System(object):
    ...
    def __init__(
        ...
        self.__api = API(
            hostname, app_id, user_code, user_email, user_password, panel_id, partition
        )
    ...
    def connect(self):
        ...
        if isinstance(rest_versions, list) and len(rest_versions) > 0:
            latest_version = max(rest_versions)
            self.__api.setVersionUrls(latest_version)
        else:
            raise Exception(
                f"Rest API version 8.0, 9.0 or 10.0 is not supported by server. Supported versions: {', '.join(rest_versions)}"
            )
    ...
    def update_status(self):
        ...
        if alarms_events is None or len(alarms_events) == 0:
            if partition["status"] == "EXIT" and (
                partition["state"] == "AWAY" or partition["state"] == "HOME"
            ):
            ...
            if device is not None:
                if device["subtype"] is not None:
                    if "CONTACT" in device["subtype"]:
                        ...
                        device["subtype"] == "MOTION" or device["subtype"] == "CURTAIN"
                    ):
...
class API(object):
    ...
    def get_version_info(self):
        ...
        return self.__send_get_request(
            self.__url_version, with_user_token=False, with_session_token=False
        )
    ...
    def get_status(self):
        ...
        return self.__send_get_request(
            self.__url_status, with_user_token=True, with_session_token=True
        )
    ...
    def get_alarms(self):
        ...
        return self.__send_get_request(
            self.__url_alarms, with_user_token=True, with_session_token=True
        )
    ...
    def get_troubles(self):
        ...
        return self.__send_get_request(
            self.__url_troubles, with_user_token=True, with_session_token=True
        )
    ...
    def get_alerts(self):
        ...
        return self.__send_get_request(
            self.__url_alerts, with_user_token=True, with_session_token=True
        )

    def get_panel_info(self):
        """Get the panel information."""
        return self.__send_get_request(
            self.__url_panel_info, with_user_token=True, with_session_token=True
        )

    def get_events(self):
        """Get the alarm panel events."""
        return self.__send_get_request(
            self.__url_events, with_user_token=True, with_session_token=True
        )
    ...
    def get_wakeup_sms(self):
        ...
        return self.__send_get_request(
            self.__url_wakeup_sms, with_user_token=True, with_session_token=True
        )
    ...
    def get_all_devices(self):
        ...
        return self.__send_get_request(
            self.__url_all_devices, with_user_token=True, with_session_token=True
        )

    def get_locations(self):
        """Get all locations in the alarm system."""
        return self.__send_get_request(
            self.__url_locations, with_user_token=True, with_session_token=True
        )

    def arm_home(self, partition):
        """Arm in Home mode and with Exit Delay."""
        arm_info = {"partition": -1, "state": "HOME"}
        arm_json = json.dumps(arm_info, separators=(",", ":"))

        return self.__send_post_request(
            self.__url_set_state,
            arm_json,
            with_user_token=True,
    ...
    def get_process_status(self, token):
        res = self.__send_get_request(
            self.__url_process_status + "?process_tokens=" + token,
            with_user_token=True,
            with_session_token=True,
        )
    ...
    def arm_away(self, partition):
        ...
        arm_info = {"partition": -1, "state": "AWAY"}
        arm_json = json.dumps(arm_info, separators=(",", ":"))
    ...
    def disarm(self, partition):
        ...
        disarm_info = {"partition": -1, "state": "DISARM"}
